...
We do not recommend Windows for brtd production use at this time. Currently,
...
modification, build brtd under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build brtd. The **Visual
...
* NOTE: Since brtd links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. brtd does not use
...
The brtd application is linked statically to the standard runtimes and
...
process and clone `git@github.com:ripple/brtd.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone brtd (git bash uses
...
5. Select either the `brtd.exe` (unity) or `brtd_classic.exe` (non-unity)
   ...
   brtd repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\brtd.exe
```
or
```
.\build\x64-Debug\Debug\brtd.exe
```
...
```
.\build\cmake\Release\brtd.exe
```
or
```
.\build\cmake\Debug\brtd.exe
```
These paths are relative to your cloned git repository.

# Unity/No-Unity Builds

The brtd build system defaults to using [unity source files](http://onqtam.com/programming/2018-07-07-unity-builds/)
...
`brtd` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`brtd` executable. The executable will exit with summary info after running
the unit tests.
