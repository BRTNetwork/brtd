...
# auto-update script for brtd daemon
...
UPDATELOG=/var/log/brtd/update.log
...
if [[ "$ID" == "ubuntu" || "$ID" == "debian" ]] ; then
  ...
  RIPPLE=$(apt-get install -s --only-upgrade brtd | awk '/^Inst/ { print $2 }')
  test "$RIPPLE" == "brtd" && can_update=true

  function apply_update {
    apt-get install brtd -qq
  }
  ...
  yum check-update -q --enablerepo=ripple-$RIPPLE_REPO brtd || can_update=true

  function apply_update {
    yum update -y --enablerepo=ripple-$RIPPLE_REPO brtd
  }
else
...
if [ "$can_update" = true ] ; then
  ...
  systemctl restart brtd.service
  echo $(date -u) "brtd daemon updated."
else
