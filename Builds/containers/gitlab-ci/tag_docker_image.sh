...
set -ex
docker login -u brtd \
    -p ${ARTIFACTORY_DEPLOY_KEY_brtd} "${ARTIFACTORY_HUB}"
# this gives us brtd_version :
...
# tag/push two labels...one using the current brtd version and one just using "latest"
for label in ${brtd_version} latest ; do
    docker tag \
