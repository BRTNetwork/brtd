...
if [ "$use_private" -gt 0 ] ; then
    REPO_ROOT="https://brtd:${ARTIFACTORY_DEPLOY_KEY_brtd}@${ARTIFACTORY_HOST}/artifactory"
else
...
if [ "${pkgtype}" = "dpkg" ] ; then
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        # apt-cache policy brtd
        apt-get -y install brtd=${dpkg_full_version}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        updateWithRetry
        apt-get -y install libprotobuf-dev libssl-dev
        rm -f build/dpkg/packages/brtd-dbgsym*.*
