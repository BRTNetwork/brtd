...
**`brtd`** or other compatible software. Each peer maintains multiple
...
where vertices are instances of `brtd` and edges are persistent TCP/IP
...
```
GET / HTTP/1.1
User-Agent: brtd-1.4.0-b1+DEBUG
...
Cluster members are configured in the `brtd.cfg` file under
