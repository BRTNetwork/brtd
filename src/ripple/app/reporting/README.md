Reporting mode is a special operating mode of brtd, designed to handle RPCs
...
to the p2p network. To run brtd in reporting mode, you must also run a
separate brtd node in p2p mode, to use as an ETL source. Multiple reporting
...
```
$ cqlsh [host] [port]
> CREATE KEYSPACE brtd WITH REPLICATION =
...
the normal brtd behavior, where the open ledger is used by default.
