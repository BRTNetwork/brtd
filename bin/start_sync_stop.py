#!/usr/bin/env python
"""A script to test brtd in an infinite loop of start-sync-stop.
...
DEFAULT_EXE = 'brtd'
DEFAULT_CONFIGURATION_FILE = 'brtd.cfg'
...
async def brtd(exe=DEFAULT_EXE, config_file=DEFAULT_CONFIGURATION_FILE):
    """A context manager for a brtd process."""
    # Start the server.
    ...
    logging.info(f'brtd started with pid {process.pid}')
    try:
        yield process
    finally:
        # Ask it to stop.
        logging.info(f'asking brtd (pid: {process.pid}) to stop')
        start = time.time()
        process.terminate()
        ...
        try:
            ...
            logging.warning(f'killing brtd ({process.pid})')
            try:
                process.kill()
        ...
        logging.info(
            f'brtd stopped after {end - start:.1f} seconds with code {code}'
        )
...
async def sync(
    ...
    """Poll brtd on an interval until it has been synced for a duration."""
...
async def loop(test,
    ...
    """
    Start-test-stop brtd in an infinite loop.
    ...
    while True:
        ...
        async with brtd(exe, config_file) as process:
            start = time.perf_counter()
...
try:
    asyncio.run(loop(test, exe=args.brtd, config_file=args.conf))
