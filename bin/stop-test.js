...
if (process.argv[2] == null) {
  [
   'Usage: ',
   '',
   '  `node bin/stop-test.js i,j [brtd_path] [brtd_conf]`',
   '',
   '  Launch brtd and stop it after n seconds for all n in [i, j}',
   '  For all even values of n launch brtd with `--fg`',
   '  For values of n where n % 3 == 0 launch brtd with `--fg`\n',
   'Examples: ',
   '',
   '  $ node bin/stop-test.js 5,10',
   ('  $ node bin/stop-test.js 1,4 ' +
      'build/clang.debug/brtd $HOME/.confs/brtd.cfg')
   ]
  ...
  var brtdPath = process.argv[3] || 'build/brtd'
  var brtdConf = process.argv[4] || 'brtd.cfg'
}
...
function start(args) {
    return child.spawn(brtdPath, args, options);
}
function stop(brtd) { child.execFile(brtdPath, stop_args, options)}
...
(function oneTest(n /* seconds */, results) {
  ...
  console.log("\nLaunching `%s` with `%s` for %d seconds",
                brtdPath, JSON.stringify(args), n);

  brtd = start(args);
  console.log("brtd pid: %d", brtd.pid);
  ...
  secs_l8r(n, function(){
    console.log("Stopping brtd after %d seconds", n);
