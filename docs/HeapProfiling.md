## Heap profiling of brtd with jemalloc
...
jemalloc is acquired separately from brtd, and is not affiliated
...
profiling has been tested with brtd on a Linux platform. It should
work on platforms on which both brtd and jemalloc are available.

To link brtd with jemalloc, the argument
...
Build brtd with jemalloc library under /usr/local/lib and
header under /usr/local/include:

    $ scons profile-jemalloc=/usr/local

Build brtd using clang with the jemalloc library under /opt/local/lib
